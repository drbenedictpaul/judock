# judock.jl (Production Version - No Auto-Install)

# --- Load Packages (Assumes they are pre-installed in Docker) ---
...
route("/start-pipeline", method = "POST") do
    ...
    if isempty(ligand_files); return json(Dict("success" => false, "message" => "Input directory is empty. Please place .sdf files in ~/juDock_input")); end
    ...
    @async begin
        ...
        # Immediate sleep to let server respond to frontend
        sleep(0.2)
...
println("                                                      ")
println("                   Surya Sekaran                      ")
println("            Dr. Benedict Christopher Paul             ")
